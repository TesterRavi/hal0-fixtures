...
FOR EACH ROW
  BEGIN
    ...
    INSERT INTO solr_queue_flash(sku, triggered_at) VALUES (NEW.sku, NOW());
  END */;;
DELIMITER ;
...
FOR EACH ROW
  BEGIN
    IF
      NOT (NEW.sold <=> OLD.sold)
      OR NOT (NEW.available <=> OLD.available)
      OR NOT (NEW.returned <=> OLD.returned)
    THEN
      INSERT INTO solr_queue_inventory (sku, event_id, triggered_at) VALUES (NEW.sku, NEW.event_id, NOW());
    END IF;
    ...
    IF NOT (NEW.active <=> OLD.active)
      ...
      OR NOT (NEW.sale_price <=> OLD.sale_price)
      OR NOT (NEW.retail_price <=> OLD.retail_price)
      OR NOT (NEW.discount <=> OLD.discount)
    THEN
      INSERT INTO solr_queue_flash(sku, triggered_at) VALUES (NEW.sku, NOW());
    END IF;
  END */;;
...
FOR EACH ROW
  BEGIN
    ...
    INSERT INTO solr_queue_flash(sku, triggered_at) VALUES (OLD.sku, NOW());
  END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
